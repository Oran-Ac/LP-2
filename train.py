...
# new parameters
flags.DEFINE_float('new_lr', None, 'New learning rate')
flags.DEFINE_string('new_optimizer', None, 'New optimizer type')
flags.DEFINE_float('new_weight_decay', None, 'New weight decay')
# save
...
def data4curriculum_learning(data_dict, tokenizer_type, max_length, augment_prob=0.3, categories=None):
    ...
    for c in categories:
        ...
        # 调用 prepare_data 函数进行数据增强
        prepare_data(data_dict, c, 'train', tokenizer_type, max_length, augment_prob)
        prepare_data(data_dict, c, 'validation', tokenizer_type, max_length, augment_prob)
...
def main(argv):
    ...
    if FLAGS.new_lr is not None:
        FLAGS.lr = FLAGS.new_lr
    if FLAGS.new_optimizer is not None:
        FLAGS.optimizer = FLAGS.new_optimizer
    if FLAGS.new_weight_decay is not None:
        FLAGS.weight_decay = FLAGS.new_weight_decay

    # load the data
    if FLAGS.curriculum_learning:
        for train_dataset, validation_dataset, c in data4curriculum_learning(FLAGS.data_dict, FLAGS.model_type, FLAGS.max_length, augment_prob=0.3):
            # data loader
            ...
            for epoch in range(FLAGS.num_epochs):
                ...
                if acc > best_acc:
                    best_acc = acc
                    if accelerator.is_main_process:
                        model_save_path = os.path.join(FLAGS.output_model_dir, f'{FLAGS.model_type.replace("/", "-")}_{c}')
                        model.save_pretrained(model_save_path)
                        logger.info(f'Save the model at {model_save_path}')
                else:
                    patience_counter += 1
