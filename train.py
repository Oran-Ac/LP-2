...
# hybrid data
flags.DEFINE_boolean('hybrid_flag',False,'hybrid data')
...
def data4curriculum_learning(data_dict,tokenizer_type,max_length,hybrid_flag):
    categories = ['easy_medium','medium_hard','hard']
    logger.info('Start preparing the data for curriculum learning')
    for c in categories:
        logger.info(f'Preparing the data for {c}')
        train_dataset = ClassificationDataset(data_dict,c,'train',tokenizer_type,max_length,hybrid_flag)
        validation_dataset = ClassificationDataset(data_dict,c,'validation',tokenizer_type,max_length,hybrid_flag)
...
def main(argv):
    ...
    if FLAGS.curriculum_learning:
        for train_dataset,validation_dataset in data4curriculum_learning(FLAGS.data_dict,FLAGS.model_type,FLAGS.max_length,FLAGS.hybrid_flag):
            # data loader
